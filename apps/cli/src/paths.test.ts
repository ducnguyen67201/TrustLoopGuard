...
import { canonicalizeProject, ensureOwnedDirectory, resolveTrustLoopPaths } from './paths.js';
...
describe('paths', () => {
  test('resolves XDG, host, and CODEX_HOME locations', () => {
    const paths = resolveTrustLoopPaths(
      {
    ...
    expect(paths.registryFile).toBe('/config/trustloopguard/registry.json');
    ...
    expect(paths.openCodePluginFile).toBe('/config/opencode/plugins/trustloopguard.mjs');
  });
  ...
  test('resolves Windows AppData without changing environment variables', () => {
    ...
    const paths = resolveTrustLoopPaths(env, 'win32', 'C:\\Users\\A');
    expect(paths.configRoot).toContain('trustloopguard');
    expect(env.CODEX_HOME).toBe('C:\\Codex');
  });

  test('canonicalizes projects and rejects symlinked owned directories', async () => {
    const parent = await mkdtemp(join(tmpdir(), 'tlg-paths-'));
