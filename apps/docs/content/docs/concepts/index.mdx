...
| Lease                 | A one-attempt execution right issued only after TrustLoopGuard rechecks current policy and live state.                     |
...
1. The agent proposes an executable action and TrustLoopGuard records its intent.
...
5. The caller retries with that grant; TrustLoopGuard rechecks current policy and live state.
6. A one-attempt lease prevents duplicate execution.
7. TrustLoopGuard writes an authorization receipt, while the domain records its own result.
