---
title: Glossary
description: Every TrustLoopGuard domain and technical term defined once.
---

If you find yourself explaining a term in a PR review, add it here instead.
...
Example: see [`policies/refund-promise.yaml`](https://github.com/ducnguyen67201/TrustLoopGuard/blob/main/policies/refund-promise.yaml).
...
When multiple policies trigger, the engine picks the most severe action. Today the engine takes the first triggered policy; richer multi-policy merge is on the roadmap.
...
The committed p99 for each channel. See [Architecture · Latency budget](/docs/concepts/architecture#latency-budget-committed). Treat as a contract.
