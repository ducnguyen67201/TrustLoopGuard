...
description: The single signature every TrustLoopGuard host adapter mirrors, in every language.
---

If you're writing a new SDK or a new host integration (OpenAI middleware, LiveKit, Pipecat, Vapi, custom), this is the shape.
...
Same as `tl-core::Decision`. See [Glossary · Decision](/docs/concepts/glossary#decision).
...
4. **Respect `fail_open` vs `fail_closed`** on transport errors. Per-policy config in v2; per-client config in v1. Default = fail-closed. See [Glossary · Fail-open vs fail-closed](/docs/concepts/glossary#fail-open-vs-fail-closed).
...
1. Generate types from [`crates/tl-core/src/lib.rs`](https://github.com/ducnguyen67201/TrustLoopGuard/blob/main/crates/tl-core/src/lib.rs) or hand-write them — but they must serialize to the same JSON.
2. Implement `Guard.check(draft, ctx)` over HTTP using the OpenAPI spec at [`docs/openapi.yaml`](https://github.com/ducnguyen67201/TrustLoopGuard/blob/main/docs/openapi.yaml).
