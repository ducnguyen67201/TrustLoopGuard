...
export function createHealthcareRuntimeClient(
  options: { serverUrl?: string; runtimeApiKey?: string; fetchImpl?: typeof fetch } = {},
): Client {
  const runtimeApiKey = (options.runtimeApiKey ?? HEALTHCARE_DEMO_API_KEY)?.trim();
  if (runtimeApiKey === undefined || runtimeApiKey === '') {
    throw new Error('TL_HEALTHCARE_DEMO_API_KEY is required for the hosted healthcare demo');
  }
  if (!runtimeApiKey.startsWith('tl_live_')) {
    throw new Error('TL_HEALTHCARE_DEMO_API_KEY must be a workspace runtime key');
  }
  return new Client({
