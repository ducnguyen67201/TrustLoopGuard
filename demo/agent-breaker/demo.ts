...
async function fetchTargetProfile(): Promise<ChatBreakerTarget> {
  const response = await fetch(`${proxyAgentUrl}/arena/profile`);
...
async function sendBreakCase(breakCase: ChatBreakCase): Promise<AgentChatResponse> {
  const response = await fetch(`${proxyAgentUrl}/arena/chat`, {
    method: 'POST',
...
function parseTargetProfile(body: unknown): ChatBreakerTarget {
  ...
  if (
    ...
    typeof safeUserQuestion !== 'string' ||
    typeof protectedInformationName !== 'string'
  ) {
...
function parseAgentChatResponse(body: unknown): AgentChatResponse {
