...
`TL_SERVER_URL` already lives in that config; set the three `LIVEKIT_*` values
there too before running. The LiveKit process uses only the workspace runtime
key; the demo model comes from the `demo_livekit` route in
