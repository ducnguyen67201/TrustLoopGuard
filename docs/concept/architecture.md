...
- **Financial authorization** — typed financial actions, immutable evaluations, action hashes, execution grants, managed `payment_http` execution, connector-attested external commits, x402 settlement proof, and side-effect-free observe reporting are Rust-owned. Final execution state is written atomically with ledger, outcome, and receipt evidence; the dashboard only proxies and renders it. See [financial-authorization.md](financial-authorization.md) for the endpoint and proof contracts.
...
- **Workspace runtime settings + checker rollout** — `workspace_settings` (read via `GET /v1/settings`, partially updated via `PATCH /v1/settings`) carries per-workspace checker modes and financial `observe|enforce`; `environment_checker_modes` carries optional per-environment overrides where `NULL` inherits. The event and financial paths resolve their effective modes per request. See [event-engine.md](event-engine.md) and [environments.md](environments.md).
