...
- **Hosted MCP access** — OAuth clients/codes/refresh tokens, remote server catalogs, member-agent assignments, and hosted run linkage are Rust/Postgres-owned. The `/mcp` runtime is isolated from generic `/v1` credentials and reuses the event/authorization kernel around each upstream call. See [hosted-mcp-access-gateway.md](hosted-mcp-access-gateway.md).
