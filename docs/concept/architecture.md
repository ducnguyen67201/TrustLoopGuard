...
The customer's agent does not stop being smart. Featherlane AI is a **gate**, not a brain. It says "this proposed output or action is fine" or "this is dangerous, here's a safer path."
...
2. **Gateway** — provider-compatible proxy endpoints under `/v1/gateway/*`. The customer routes AI traffic through Featherlane AI, and the Rust gateway applies the same composed `Decision` as `/v1/events` before returning a provider-shaped response. See [gateway.md](gateway.md).
...
7. **Coding-agent tool gate** — `@featherlane-ai/cli` installs user-level Claude Code, Codex, or OpenCode adapters that normalize host-emitted tool calls and submit them directly to `POST /v1/events`. The bridge owns no policy logic or durable product data. See [coding-agent-tool-gates.md](coding-agent-tool-gates.md).
...
- **Authorization authority** — Featherlane AI owns database-backed approvals, scoped grants, execution leases, and receipts. External identity or signature systems may supply future attestations, but no opaque bearer proof bypasses the kernel. See [authorization-kernel.md](authorization-kernel.md).
...
- **Runs** — one execution of a registered customer agent, such as a chat session, live call, workflow execution, or background job. Runs are surfaced through `/v1/runs/*` and group persisted decision traces through `traces.run_id`. Ordered run events are stored in `run_events`; normalized OTLP spans are stored in `run_spans`; `GET /v1/runs/{run_id}` returns both for the dashboard's audit log and execution waterfall. TypeScript `guardAgent(...)` defaults to one automatic `chat_session` Run per reply, records input/output turn events, and can bind one longer-lived Run to an explicit framework session lifecycle; explicit SDK scopes and gateway grouping remain supported. Run evidence is observational, while authorization remains on proposed tool/action/output events. Featherlane AI does not orchestrate customer agents or workflows. See [runs.md](runs.md), [telemetry-capture.md](telemetry-capture.md), and [sdk-agent-adapters.md](sdk-agent-adapters.md).
- **Red-team dispatch jobs** — durable single-target attack jobs in `redteam_jobs` + `redteam_attack_sessions` + `redteam_session_events`, surfaced through `/v1/redteam/*`. Rust binds registered dispatch targets to the selected workspace agent's stored endpoint (with only the fixed local demo adapter permitted unregistered), then persists the job, each independent attack session, and ordered transcript events while an in-process worker calls a compatible private runner (`REDTEAM_RUNNER_URL`); dispatch returns a `jobId` immediately. The dashboard Attacks tab dispatches, polls, lists, and cancels. See [redteam-dispatch.md](redteam-dispatch.md).
...
- **Authorization activity** — Rust owns receipts and the environment-scoped `GET /v1/authorization/receipts` list. The dashboard's Authorization page displays recent outcomes separately from the actionable approval queue. See [authorization-kernel.md](authorization-kernel.md).
- **Workspace policies** — policy authoring, listing, editing, delete, and enablement changes are Rust-owned through `/v1/policies/*`. Policy definitions are workspace-level, carry a `family` discriminator, and use environment-scoped deployment state for runtime enablement. Runtime checks only load policies enabled for the resolved environment. Mutation authorization is defined in [policies.md](policies.md); workspace runtime keys cannot change governing policies. Workspace creation seeds disabled starter policies that authorized users can enable, edit, or delete like any other policy.
- **Knowledge sources** — durable workspace knowledge is Rust-owned through `/v1/knowledge-sources`. Reads remain available to workspace runtime consumers; creation requires an authenticated workspace Owner/Admin, and the dashboard forwards the signed-in identity rather than elevating a membership-only server action.
...
- **Source label policies** — workspace-scoped per-origin label overrides are `family: source_label` policies in the unified registry. `/v1/label-policies` remains an ergonomic compatibility wrapper, while the event pipeline reads the same registry-backed label policy state for label resolution. Runtime keys may read resolved policy effects but cannot mutate this control-plane configuration. See [event-engine.md](event-engine.md) and [policies.md](policies.md).
- **Agent profiles** — Rust owns workspace agent profiles and agent-owned policy cascades. Runtime keys cannot create, replace, or delete profiles; in particular, deleting an agent cannot be used as a runtime-key path to delete another agent's policies.
...
- **Workspace team, invites, lifecycle, and dashboard feature rollout** — Rust owns this durable control-plane state and its authorization boundaries. See [team-and-invites.md](team-and-invites.md) for the canonical behavior.
- **Hosted MCP access** — OAuth clients/codes/refresh tokens, remote server catalogs, member-agent assignments, and hosted run linkage are Rust/Postgres-owned. OAuth code issuance accepts only the trusted dashboard service identity; runtime keys cannot cross into the user-token lane. The `/mcp` runtime is isolated from generic `/v1` credentials and reuses the event/authorization kernel around each upstream call. See [hosted-mcp-access-gateway.md](hosted-mcp-access-gateway.md).
