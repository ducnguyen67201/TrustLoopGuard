...
- **Trace detail/export** — point lookup for persisted decision traces is Rust-owned through `GET /v1/traces/{trace_id}`. The web dashboard renders the stored trace evidence and raw JSON export; it does not replay policy execution or store trace data in the web app.
