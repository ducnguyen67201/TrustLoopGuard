...
6. **Hosted MCP access gateway** — an employee points an MCP-capable AI client at one managed `/mcp` endpoint. OAuth binds employee and workspace identity, durable assignments filter the catalog, and Rust policy enforcement gates each upstream call. See [hosted-mcp-access-gateway.md](hosted-mcp-access-gateway.md).
...
- **Workspace team, invites, lifecycle, and dashboard feature rollout** — Rust owns workspace membership, owner-authorized soft deletion, and the atomic revocation of pending invites and active runtime keys. `workspaces` also carries default-off dashboard availability flags returned by `GET /v1/team/my-workspaces`. See [team-and-invites.md](team-and-invites.md) for the canonical lifecycle behavior.
- **Hosted MCP access** — OAuth clients/codes/refresh tokens, remote server catalogs, and member tool assignments are Rust/Postgres-owned. The `/mcp` runtime is isolated from generic `/v1` credentials and reuses the event/authorization kernel before any upstream call. See [hosted-mcp-access-gateway.md](hosted-mcp-access-gateway.md).
