...
- **Workspace team, invites, lifecycle, and dashboard feature rollout** — Rust owns workspace membership, the default-false `users.is_platform_admin` cross-workspace support gate, owner-authorized soft deletion, and the atomic revocation of pending invites and active runtime keys. `GET /v1/team/my-workspaces` returns `is_platform_admin` plus either the caller's active memberships or every active workspace for a platform administrator. `workspaces` also carries default-off dashboard availability flags. See [team-and-invites.md](team-and-invites.md) for the canonical lifecycle behavior.
