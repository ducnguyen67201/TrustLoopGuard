...
The `tl-llm` chokepoint for first-party provider/model dispatch. It applies typed workload routes, deadlines, optional reasoning, primary/fallback behavior, and telemetry. Runtime judge calls retain atomic per-tenant `TokenBudget` admission and settlement; control-plane route calls are not charged to that runtime budget. The versioned configuration and consumer boundary are defined in [first-party LLM routing](llm-routing.md). Customer gateway routing is separate.
