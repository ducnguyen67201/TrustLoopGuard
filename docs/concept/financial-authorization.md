...
- `financial_receipts` stores tenant-scoped proof records for executed actions. The receipt id matches the action id and records generic execution proof, ledger entry ids, and `payment_http` provider evidence when the action executes through a vaulted provider connection.
