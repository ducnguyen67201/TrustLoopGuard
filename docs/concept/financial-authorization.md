...
- `financial_receipts` stores tenant-scoped proof records for executed actions. The receipt id matches the action id and records generic execution proof plus the ledger entry ids created during execution; provider-rich proof is added when provider execution is wired into the financial service.
