...
> Featherlane AI returns a **Decision** and **reasons**. The caller is responsible for acting on the verdict, sending messages, managing queues, and handling fallback when we are unreachable.
...
Layer 3: Helper patterns        (guardAgent(), guard(), branch helpers)
...
1. **TypeScript SDK** (`@featherlane-ai/sdk`) — typed client + `guardAgent()` and `guard()` helpers
2. **Python SDK** (`featherlane-ai`) — typed client + `@guarded` and `guard()` helpers
...
The canonical customer journey starts from the published language package, not
from a repository checkout. TypeScript users install `@featherlane-ai/sdk` and
decorate the agent once with `guardAgent(createAgent(), { agentId })`. The
decorated object preserves the original interface, so existing
`agent.reply(...)` call sites do not change. Python users apply
`@guarded(agent_id=...)` to an async reply function.

These helpers are intentionally narrow adapters over `POST /v1/events`: the
user input and returned draft are strings. The root agent decorator sees the
calls crossing `reply()` but not hidden framework internals. Tool calls,
...
`docs/research/featherlane-ai-runtime-security-architecture/main.pdf`.
...
ladder (layer 1). Measured by FeatherlaneAIBench: parameter-source catch rate
