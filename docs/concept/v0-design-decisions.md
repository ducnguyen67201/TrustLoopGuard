...
1. **TypeScript SDK** (`@trustloopguard/sdk`) — typed client + `guardAgent()` and `guard()` helpers
2. **Python SDK** (`trustloopguard`) — typed client + `@guarded` and `guard()` helpers
...
from a repository checkout. TypeScript users install `@trustloopguard/sdk` and
