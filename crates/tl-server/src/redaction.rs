...
pub fn requires_redaction_rejection(mode: DataHandlingMode, req: &CheckRequest) -> bool {
    ...
    let redaction_permitted = req.redaction.as_ref().is_some_and(|info| {
        info.status == RedactionStatus::Applied || info.mode == RedactionMode::Server
    });
    !redaction_permitted && contains_raw_sensitive_data(req)
}
...
impl Redactor {
    ...
    fn redact_text(&mut self, text: &str) -> String {
        ...
        let text = self.redact_with(income_regex(), "INCOME_AMOUNT", &text);
        self.redact_with(person_name_regex(), "PERSON_NAME", &text)
    }
...
fn person_name_regex() -> &'static Regex {
    static PERSON_NAME: OnceLock<Regex> = OnceLock::new();
    PERSON_NAME.get_or_init(|| Regex::new(r"\b[A-Z][a-z]+ [A-Z][a-z]+\b").unwrap())
}
